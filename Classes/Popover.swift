...
open class Popover: UIView {
    ...
    open var animationType: AnimationType = .popover
    ...
    fileprivate func show() {
        ...
        UIView.animate(withDuration: self.animationIn, delay: 0,
                       ...
                       animations: {
                        self.enterAnimationResult(origRect: origRect)
        }){ _ in
            self.didShowHandler?()
        }
    ...
    func enterAnimationWillStart() {
        switch self.animationType {
        case .popover:
            self.transform = CGAffineTransform(scaleX: 0.0, y: 0.0)
        case .custom:
            let origRect = self.contentView.frame
            self.contentView.frame = CGRect(x: origRect.origin.x, y: origRect.origin.y, width: origRect.size.width, height: 0)
            let contenRect = self.frame
            self.frame = CGRect(x: contenRect.origin.x, y: contenRect.origin.y, width: contenRect.size.width, height: 0)
        default: break
            
        }
    }
    
    func enterAnimationResult(origRect:CGRect) {
        switch self.animationType {
        case .popover:
    ...
    func leavAnimationDidEnd(origRect:CGRect,contenRect:CGRect) {
        switch self.animationType {
        case .popover:
            self.transform = CGAffineTransform.identity
    ...
    open func dismiss() {
        if self.superview != nil {
            ...
            UIView.animate(withDuration: self.animationOut, delay: 0,
                           options: UIViewAnimationOptions(),
                           animations: {
                            self.leaveAnimationResult()
                            self.blackOverlay.alpha = 0
            }){ _ in
                self.contentView.removeFromSuperview()
                self.blackOverlay.removeFromSuperview()
                self.removeFromSuperview()
                self.leavAnimationDidEnd(origRect: origRect, contenRect: contenRect)
